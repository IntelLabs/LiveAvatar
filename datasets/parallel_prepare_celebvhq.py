    ...
    def run(self):
        ...
        while not self.job_queue.empty():
            try:
                ...
                idx, vid_id, save_vid_name, _, _ = job_data

                log.info(f"==== Video {idx}: {save_vid_name} (Device {self.device}, {torch.cuda.current_device()}) ====")

                clip_name = os.path.splitext(save_vid_name)[0]
                video_path = os.path.join(self.cfg.video_dir, f"{clip_name}.mp4")
...
def run_singleprocessing_track(args):
    ...
    for idx, vid_id, save_vid_name, time, bbox in load_data(json_path, args.st, args.nd):
        print(f"==== Video {idx}: {save_vid_name} ====")

        clip_name = os.path.splitext(save_vid_name)[0]
        video_path = os.path.join(args.video_dir, f"{clip_name}.mp4")
...
if __name__ == '__main__':
    ...
    default_data_dir = os.path.join(file_dir, "../../../data/datasets/CelebV-HQ")
    ...
    parser.add_argument('--overwrite', type=bool_str, default=False)
    ...
    parser.add_argument('--max_frame_count', type=int, default=16)
    ...
    # parser.add_argument('-t', '--tasks',  nargs='+', default=['metadata'], metavar='N')

    args = parser.parse_args()

    if args.output_dir is None:
        args.output_dir = os.path.join(args.root, 'processed_celebvhq')
    os.makedirs(args.output_dir, exist_ok=True)

    if args.video_dir is None:
        args.video_dir = os.path.join(args.root, '35666')
